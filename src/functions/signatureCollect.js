export function sameFormItemId(a, b) {
...
export function serializeSignedDateTime(value) {
  if (!value) {
    return null
  }
...
export function isSignedSignatureValue(value) {
  return Boolean(value && typeof value === 'object' && !Array.isArray(value) && value.isSigned)
}

export function resolveColumnChild(childRef, findById) {
  if (childRef == null || childRef === '') {
    return null
  }
  if (typeof childRef === 'object') {
    if (childRef.element || childRef.field_name) {
      return (typeof findById === 'function' && childRef.id != null ? findById(childRef.id) : null) ||
        childRef
    }
    return null
  }
...
export function pickSignatureValue(refState, storedValue) {
...
export function flattenColumnChildren(data, findById) {
  const children = []
  const seen = new Set()
  ;(data || []).forEach((item) => {
