    ...
    (0, _defineProperty2.default)(this, "clickToSign", () => {
      ...
      const applySignedState = current => {
        ...
        return _objectSpread(_objectSpread({}, current), {}, {
          ...
          signedDateTime: nextIsSigned ? (0, _dayjs.default)().utc(true).toISOString() : null
        });
      };
        ...
        if (typeof this.props.onSignChange === 'function') {
          this.props.onSignChange({
            ...
            value: nextState.isSigned ? {
              ...
              signedDateTime: (0, _signatureCollect.serializeSignedDateTime)(nextState.signedDateTime)
            } : ''
          });
        }
      };
      const canSignSpecific = this.props.data.specificRole === 'specific' && roleLists.find(item => "".concat(item).toLocaleLowerCase().trim() === position);
      const canSignAnyone = this.props.data.specificRole === 'notSpecific' || this.props.data.specificRole == null || this.props.data.specificRole === '';
