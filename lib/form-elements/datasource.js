...
class DataSource extends _react.default.Component {
  constructor(props) {
    ...
    (0, _defineProperty2.default)(this, "debounceOnChange", value => {
      const matchData = this.state.sourceList.filter(item => "".concat(item.name).toLocaleLowerCase().includes("".concat(value).toLocaleLowerCase()));
      this.setState({
        searchText: value,
  ...
  render() {
    ...
    }, (this.state.matchedList || []).map(item => /*#__PURE__*/_react.default.createElement("div", {
      key: item.id,
      style: {
        position: 'relative',
        display: 'block',
