...
(0, _defineProperty2["default"])(DataSource, "getDerivedStateFromProps", function (props, state) {
  ...
  if (props.getDataSource !== state.getDataSource || props.defaultValue && JSON.stringify(props.defaultValue.selectedItem) !== JSON.stringify(state.defaultSelectedItem)) {
    console.log("DataSource >> getDerivedStateFromProps => props changed");
    var defaultValue = props.defaultValue || {};
    var data = [];

    if (props.getDataSource && typeof props.getDataSource === "function") {
      data = DataSource.props.getDataSource(props.data.sourceType);
    }
