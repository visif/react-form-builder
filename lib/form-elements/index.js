...
class Header extends _react.default.Component {
  render() {
    ...
    const rawContent = String(this.props.data.content || '').replace(/<[^>]+>/g, '').trim();
    const generateSectionID = this.props.generateSectionID;
    const sectionId = typeof generateSectionID === 'function' && rawContent ? generateSectionID(rawContent) : undefined;
