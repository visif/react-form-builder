...
function effectiveCellName(child) {
  ...
  var named = sanitizeCellName(child.cellName);
  if (named) {
    return named;
  }
...
function isCellNameTaken(data, parentId, cellName, exceptId) {
  var wanted = sanitizeCellName(cellName);
  if (!wanted || !parentId) {
    return false;
  }
  return (Array.isArray(data) ? data : []).some(function (item) {
    return item && item.parentId === parentId && item.id !== exceptId && effectiveCellName(item) === wanted;
  });
}
function templateTagPreview(tableUniqueName, cellName) {
  var table = sanitizeUniqueName(tableUniqueName) || 'DynamicColumnRow1';
  var cell = sanitizeCellName(cellName) || 'r1c1';
