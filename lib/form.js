...
class ReactForm extends _react.default.Component {
  constructor(props) {
    ...
    (0, _defineProperty2.default)(this, "_handleSignature2Change", () => {
    ...
    if (sectionItems.length > 0) {
      ...
      reverseKeys.forEach(key => {
        ...
        if (key && !activeSectionFound) {
          fillingItems = items.find(item => item.element !== 'Table' && item.element !== 'Dropdown' && item.element !== 'Range' && (Array.isArray(item.value) && item.value.length > 0 || typeof item.value !== 'object' && !Array.isArray(item.value) && !!item.value || item.element === 'FileUpload' && item.value.fileList && item.value.fileList.length > 0 || item.element === 'ImageUpload' && !!item.value.filePath));
  ...
  getContainerElement(item, Element) {
    ...
    const renderCell = x => {
      ...
      if (!(x && currentItem)) {
        return /*#__PURE__*/_react.default.createElement("div", null, "\xA0");
      }
      const control = this.getInputElement(currentItem);
      if (isDynamicColumnRow && currentItem.required === true && currentItem.displayLabelInColumn !== true && !readOnly && !currentItem.readOnly) {
    ...
    const controls = Array.isArray(item.childItems[0]) ? item.childItems.map(row => row.map(renderCell)) : [item.childItems.map(renderCell)];
