  ...
  function _isIncorrect(item) {
    ...
    if (item.canHaveAnswer) {
      ...
      if (item.element === "Checkboxes" || item.element === "RadioButtons") {
        item.options.forEach(function (option) {
          var $option = _reactDom["default"].findDOMNode(_ref.options["child_ref_".concat(option.key)]);
  ...
  function _isInvalid(item) {
    ...
    if (item.required === true) {
      ...
      if (item.element === "Checkboxes" || item.element === "RadioButtons") {
        ...
        item.options.forEach(function (option) {
          var $option = _reactDom["default"].findDOMNode(_ref2.options["child_ref_".concat(option.key)]);
          if ($option.checked) {
            checked_options += 1;
          }
        });
