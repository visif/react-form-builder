...
var ReactForm = /*#__PURE__*/function (_React$Component) {
  ...
  (0, _createClass2["default"])(ReactForm, [{
    ...
    value: function _collect(item) {
      ...
      var activeEditor = this._getEditor(item);
      ...
      if (item.element === "Checkboxes" || item.element === "RadioButtons") {
        ...
        itemData.editor = !activeEditor && checked_options.length > 0 ? activeUser : null;
      } else {
        if (!ref) {
          return null;
        }
        ...
        if (item.element === "Signature2") {
          itemData.editor = !activeEditor && valueItem.value.isSigned ? activeUser : null;
        } else if (item.element === "DataSource" && ref.state.searchText) {
          itemData.editor = !activeEditor && valueItem.value.value ? activeUser : null;
        } else if (item.element === "FileUpload") {
          itemData.editor = !activeEditor && valueItem.value.fileList && valueItem.value.fileList.length ? activeUser : null;
        } else if (item.element === "ImageUpload") {
          itemData.editor = !activeEditor && valueItem.value.filePath ? activeUser : null;
        }
      }
    ...
    value: function handleSubmit(e) {
      ...
      var onSubmit = this.props.onSubmit; // submit with no form
      ...
      if (onSubmit) {
        ...
        if (!this.props.skip_validations) {
          _errors = this.validateForm(); // Publish errors, if any.
    ...
    value: function validateForm() {
      ...
      var errors = [];
      var data_items = this.props.data; // re-order items to avoid items inside
      ...
      if (sectionItems.length > 0) {
        ...
        var sectionGroup = {};
        sectionGroup[activeSectionKey] = []; // group items by section separator
        ...
        var activeItems = []; // find only active section => there is any item with value input
        ...
        reverseKeys.forEach(function (key) {
          ...
          var fillingItems = items; // incase of section separator

          if (key && !activeSectionFound) {
            fillingItems = items.find(function (item) {
              return item.element !== "Table" && item.element !== "Dropdown" && item.element !== "Range" && (Array.isArray(item.value) && item.value.length > 0 || !Array.isArray(item.value) && !!item.value || item.element === "FileUpload" && item.value.fileList && item.value.fileList.length > 0 || item.element === "ImageUpload" && !!item.value.filePath);
            });
