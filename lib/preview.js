...
var _useUndoRedo = _interopRequireWildcard(require("./functions/useUndoRedo"));
...
const Preview = props => {
  ...
  const [answerData, setAnswerData] = (0, _react.useState)({});
  const editForm = (0, _react.useRef)(null);
  const {
    index: historyIndex,
    updateState
  } = (0, _useUndoRedo.default)();
  let seq = 0;
  (0, _react.useEffect)(() => {
    const {
      onLoad,
    ...
    setAnswerData({});
    seq = 0;
    _store.default.subscribe(state => {
  ...
  const _onChange = payload => {
    ...
    if (action !== _useUndoRedo.ACTION.UNDO && action !== _useUndoRedo.ACTION.REDO) {
  ...
  const swapChildren = (item, child, row, col) => {
    ...
    if (item.element === 'DynamicColumnRow') {
      (0, _dynamicColumnRowNames.applyAutoCellNamesAfterSwap)(child, row, col, oldItem, oldRow, oldCol);
    }
    _store.default.dispatch('updateOrder', data);
    return true;
  };
  const setAsChild = (item, child, row, col) => {
    ...
    if (item.element === 'DynamicColumnRow') {
      ...
      (0, _dynamicColumnRowNames.applyAutoCellName)(child, row, col);
    ...
    if (item.childItems.length > 1 && item.columns[col].isSync !== false) {
      ...
      for (let rowIndex = 0; rowIndex < item.childItems.length; rowIndex++) {
        ...
        if (rowIndex !== row) {
          ...
          const newElement = _objectSpread(_objectSpread({
            ...
            hideLabel: true,
            cellName: (0, _dynamicColumnRowNames.defaultCellName)(rowIndex, col),
          ...
          const propertiesToCopy = ['label', 'required', 'bold', 'italic', 'center', 'className', 'inline', 'readOnly', 'canHaveDisplayHorizontal', 'content', 'showDescription', 'description', 'text', 'showTimeSelect', 'showTimeSelectOnly', 'step', 'min_value', 'max_value', 'customCSS', 'defaultValue', 'default_today', 'position', 'specificRole'];
            ...
            default:
              ...
              if (child.options) {
                ...
                if (Array.isArray(newElement.options)) {
                  newElement.options = newElement.options.map(opt => _objectSpread(_objectSpread({}, opt), {}, {
                    key: "".concat(timestamp, "_").concat(Math.random().toString(36).substring(2, 9))
                  }));
                }
              }
  ...
  const removeChild = function (item) {
  ...
  const insertCard = (item, hoverIndex, id) => {
    if (id) {
      ...
      const newData = (0, _immutabilityHelper.default)(data, {
  ...
  const moveCardFromCell = (dragIdOrIndex, hoverIndex, parentId, row, col) => {
    ...
    const dragIndex = typeof dragIdOrIndex === 'number' ? dragIdOrIndex : data.findIndex(x => x && x.id === dragIdOrIndex);
    ...
    const newDragCard = _objectSpread({}, dragCard);
    ...
    const newData = (0, _immutabilityHelper.default)(data, {
      $splice: [[dragIndex, 1], [hoverIndex, 0, newDragCard]]
    });
  ...
  const syncRowChanges = changedElement => {
    ...
    const isSelectionChange = (changedElement.element === 'RadioButtons' || changedElement.element === 'Checkboxes') && changedElement.options && changedElement.options.some(opt => opt.checked);

    // For DataSource, only sync user selections, not initialization
    const isDataSourceUserSelection = changedElement.element === 'DataSource' && changedElement.isUserSelection === true;
    ...
    parentElement.childItems.forEach((row, rowIndex) => {
      ...
      const updatedItem = _objectSpread({}, itemData);
      let changed = false;
      const commonProps = ['label', 'required', 'readOnly', 'description', 'showDescription', 'className', 'customCSS', 'formSource'];
      ...
      switch (changedElement.element) {
        ...
        case 'RadioButtons':
          if (changedElement.options && itemData.options) {
            ...
            if (newOptions.length !== itemData.options.length || newOptions.some((opt, i) => {
  ...
  const getElement = (item, index) => {
    ...
    return /*#__PURE__*/_react.default.createElement(SortableFormElement, {
      ...
      onUploadFile: file => "".concat(file.name, "-").concat(Math.random() * 10000000),
      onUploadImage: file => "path/".concat(file.name, "-").concat(Math.random() * 10000000),
      onDownloadFile: file => "download_".concat(file.name, "-").concat(Math.random() * 10000000)
    });
  };
  const showEditForm = () => {
  ...
  const items = data.filter(item => !!item && !item.parentId).map((item, index) => getElement(item, index));
...
Preview.defaultProps = {
  ...
  renderEditForm: props => /*#__PURE__*/_react.default.createElement(_formElementsEdit.default, props)
};
