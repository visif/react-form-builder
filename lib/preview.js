    ...
    if (item.childItems.length > 1 && item.columns[col].isSync !== false) {
      ...
      for (let rowIndex = 0; rowIndex < item.childItems.length; rowIndex++) {
        ...
        if (rowIndex !== row) {
          ...
          const propertiesToCopy = ['label', 'required', 'bold', 'italic', 'center', 'className', 'inline', 'readOnly', 'canHaveDisplayHorizontal', 'content', 'showDescription', 'description', 'text', 'showTimeSelect', 'showTimeSelectOnly', 'step', 'min_value', 'max_value', 'customCSS', 'defaultValue', 'default_today'];
