...
var MultiColumnRow = function MultiColumnRow(props) {
  ...
  var hasRowLabels = data.rows && Array.isArray(data.rowLabels) && data.rowLabels.length > 0;
  ...
  return /*#__PURE__*/_react["default"].createElement("div", {
    ...
    }, data.rowLabels[rowIndex] ? data.rowLabels[rowIndex].text : "Row ".concat(rowIndex + 1)), row.map(function (item, columnIndex) {
...
var DynamicColumnRow = function DynamicColumnRow(_ref2) {
  ...
  if (!data.rowLabels) {
    data.rowLabels = Array(rows).fill().map(function (_, i) {
      return {
        text: "Row ".concat(i + 1),
        value: "row_".concat(i + 1),
        key: "row_".concat(Math.random().toString(36).substring(2, 9))
      };
    });
  } else if (data.rowLabels.length !== rows) {
    // Make sure the number of row labels matches the number of rows
    var currentLength = data.rowLabels.length;
    if (currentLength < rows) {
      // Add additional row labels if needed
      var additionalLabels = Array(rows - currentLength).fill().map(function (_, i) {
        return {
          text: "Row ".concat(currentLength + i + 1),
          value: "row_".concat(currentLength + i + 1),
          key: "row_".concat(Math.random().toString(36).substring(2, 9))
        };
      });
      data.rowLabels = [].concat((0, _toConsumableArray2["default"])(data.rowLabels), (0, _toConsumableArray2["default"])(additionalLabels));
    } else {
