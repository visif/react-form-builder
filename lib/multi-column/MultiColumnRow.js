...
const _excluded = ["data", "class_name"],
...
const MultiColumnRow = props => {
  ...
  const baseClasses = "SortableItem rfb-item ".concat(pageBreakBefore ? 'alwaysbreak' : '');
  ...
  const syncColumnChanges = (0, _useSyncColumnChanges.default)(childItems, getDataById, updateElement);
  ...
  const header = /*#__PURE__*/_react.default.createElement(_componentHeader.default, props);
  const headerWithHandle = connectDragSource ? connectDragSource(/*#__PURE__*/_react.default.createElement("div", {
  ...
  return /*#__PURE__*/_react.default.createElement("div", {
  ...
  }), data.columns.map((column, columnIndex) => /*#__PURE__*/_react.default.createElement("th", {
    ...
    dangerouslySetInnerHTML: {
      __html: stripPTags(column.text)
    }
  }), column.required && /*#__PURE__*/_react.default.createElement("span", {
    className: "label-required badge badge-danger ml-1"
  }, "Required"))))), /*#__PURE__*/_react.default.createElement("tbody", null, childItems.map((row, rowIndex) => /*#__PURE__*/_react.default.createElement("tr", {
    ...
    dangerouslySetInnerHTML: {
      __html: data.rowLabels[rowIndex] ? stripPTags(data.rowLabels[rowIndex].text) : ''
    }
    ...
    const childItem = getDataById && item ? getDataById(item) : null;
    const showRequiredBadge = data.element === 'DynamicColumnRow' && (childItem === null || childItem === void 0 ? void 0 : childItem.required) === true && (childItem === null || childItem === void 0 ? void 0 : childItem.displayLabelInColumn) !== true;
    ...
    }, showRequiredBadge && !controls && /*#__PURE__*/_react.default.createElement("span", {
      className: "label-required badge badge-danger",
      style: {
        display: 'inline-block',
        marginBottom: 4
      }
    }, "Required"), controls ? (_controls$rowIndex = controls[rowIndex]) === null || _controls$rowIndex === void 0 ? void 0 : _controls$rowIndex[columnIndex] : /*#__PURE__*/_react.default.createElement(_dustbin.default, (0, _extends2.default)({
      style: {
        width: '100%',
        maxWidth: '100%',
        boxSizing: 'border-box'
 ...
 */
const createColumnRow = function (defaultClassName, numberOfColumns) {
  let numberOfRows = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;
  return _ref => {
    let {
        data = {},
        class_name
    ...
    const className = "".concat(class_name || defaultClassName, " mb-2");
    const rows = data.rows || numberOfRows;
    if (!data.childItems) {
      data.childItems = Array(rows).fill().map(() => Array(numberOfColumns).fill(null));
 ...
 */
const createDynamicColumnRow = () => _ref2 => {
  var _data$columns;
  let {
      data = {},
      class_name
  ...
  const columns = ((_data$columns = data.columns) === null || _data$columns === void 0 ? void 0 : _data$columns.length) || 2;
  const defaultClassName = "col-md-".concat(Math.floor(12 / columns));
  const className = "".concat(class_name || defaultClassName, " mb-2");

  // Initialize or update childItems to match current rows and columns
  if (!data.childItems) {
    data.childItems = Array(rows).fill().map(() => Array(columns).fill(null));
    data.isContainer = true;
...
const DynamicColumnRow = exports.DynamicColumnRow = createDynamicColumnRow();
const TwoColumnRow = exports.TwoColumnRow = createColumnRow('col-md-6', 2);
const ThreeColumnRow = exports.ThreeColumnRow = createColumnRow('col-md-4', 3);
const FourColumnRow = exports.FourColumnRow = createColumnRow('col-md-3', 4);